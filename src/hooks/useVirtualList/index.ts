...
const useVirtualList = <T>(originalList: T[] = [], options: Options<T>): Result<T> => {
  const {
    containerTarget,
    wrapperTarget,
    itemHeight,
  ...
  const [visibleList, setVisibleList] = useState<{ data: T; index: number }[]>([]);
  ...
  useEffect(() => {
    containerRef.current = typeof containerTarget === 'function' ? containerTarget() : containerTarget.current || containerTarget;
    wrapperRef.current = typeof wrapperTarget === 'function' ? wrapperTarget() : wrapperTarget.current || wrapperTarget;
    ...
    const getItemHeight = (index: number): number => {
      return typeof itemHeight === 'function' ? itemHeight(index, originalList[index]) : itemHeight;
    };
    ...
    const updateVisibleList = () => {
      ...
      const totalHeight = originalList.reduce((sum, _, index) => sum + getItemHeight(index), 0);
  ...
  const scrollTo = (index: number) => {
    ...
    const heightBeforeIndex = originalList.slice(0, index).reduce((sum, _, idx) => sum + (typeof itemHeight === 'function' ? itemHeight(idx, originalList[idx]) : itemHeight), 0);
